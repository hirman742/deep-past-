# 二期云端 Steer 自迭代方案（修订执行版，2026-03-08）
...
- 北极星目标可以继续定到 `30`，但自动控制器不能直接拿 `30` 做停机阈值，否则只会把所有候选一锅端掉。控制器必须使用“分阶段存活阈值 + 业务分层阈值 + 人工终审”三层逻辑。
- 你已经明确说明，真实租机强度往往是单日 `8` 小时以上，且 credits 投入预期较高。因此本方案不走“极端保守省钱”路线，而走“高利用率、快裁撤、快迭代、保留主线证据链”的路线。
- 本版修订重点有三条：
  1. 统一阶段 gate 与业务分层，避免多套阈值互相打架。
  2. 明确 `stage_decision` 与 `business_status` 是两层不同输出。
  3. 将 `25 / 30 / 35` 一类分档阈值从核心阶段 gate 中拆出，改为 promote 后的业务分层与排队规则。
...
这里必须把“阶段控制器阈值”和“业务分层阈值”拆开写，避免同一套数字同时承担两套职责。

#### A. 阶段控制器阈值

| 层级 | 含义 | 建议阈值 |
|---|---|---|
| 生存 / 晋级阈值 | 候选值得继续活下去，并在 promote 后进入主线挑战层 | promote 阶段相对 incumbent 全量 baseline `geom +0.30` 以上，且其他健康与资源约束不过线 |
| 人工复核阈值 | 候选不应被机械拒绝，但也不应自动晋级为主线 | warmup 或 promote 阶段落入 review 区，需人工看质量、长度、吞吐与显存综合信号 |

#### B. 业务分层阈值

| 业务状态 | 含义 | 建议阈值 |
|---|---|---|
| `strong_signal` | 候选已显示出真实突破趋势 | 单折相对 incumbent `geom +0.50` 以上，或绝对 `geom >= 14.5` 且 BLEU / chrF++ 同步向上 |
| `freeze_candidate` | 候选足以冻结 recipe，转人类决定是否扩折或做集成 | 绝对 `geom >= 20.0`，且 BLEU / chrF++ 与输出健康指标没有炸裂 |
| `north_star` | 比赛后段综合目标 | `geom ≈ 30`，更可能依赖 recipe 跃迁、多折、集成、 rerank 和高质量增量数据 |

这里最关键的一点是：

- `+0.30` 不是最终目标，只是 promote 阶段的最低存活 / 晋级门槛。
- `+0.50` 以上的强信号属于业务状态，不等于自动终止搜索。
- `25 / 30 / 35` 这一类阈值只用于 promote 之后的业务分层与后续排队，不应拿来充当核心阶段 gate。
...
- 对 warm-start 路线，`3000` 是合理的主线设置；对 cold-start 路线，若前段 loss、长度与输出健康持续改善但主指标尚未充分起飞，应允许进入人工 review，而不是机械照抄 warm-start 的节奏直接判死

## 7. 边界条件：阶段 gate、软警告、人工复核
...
1. 阶段控制器输出：`stage_decision`

- `accept`
- `review`
- `reject`

这里的 `accept` 只表示“通过当前阶段并可继续下一步”，不等于“已经成为最终 mainline winner”。

2. promote 后业务状态输出：`business_status`
...
这里的 `25 / 30 / 35` 只用于 `business_status` 分层与后续排队，不应反向污染阶段 gate。

关于 GPU 指标，当前过渡期若脚本还只能稳定提供 `gpu_peak_utilization_pct`，可以临时用它做 gate；一旦补齐持续利用率统计，训练阶段的自动 gate 应优先切换到 `gpu_utilization_p90_pct + reserved` 的联合判断。

### 7.2 Probe 硬停边界

只要命中任意一条，probe 直接失败，`stage_decision = reject`：
...
只要命中任意一条，warmup 直接失败，`stage_decision = reject`：
...
满足任一情形，就不要让 AI 单独拍板，`stage_decision = review`：
...
如果以上情形都不命中，且未触发硬停，则 warmup 可记为 `stage_decision = accept`，进入 promote。

### 7.6 Promote 接受边界

候选进入 promote 后，只有同时满足下面条件，才有资格记为 `stage_decision = accept`，并挑战 incumbent：
...
### 7.7 Promote 后业务状态：强信号、冻结候选与继续观察

建议在文档层面把 promote 后的业务状态拆成三层，不再让它和 `stage_decision` 混用：

1. `observe`

- 通过 promote 阶段 gate，但还未达到强信号。
- 可保留证据链，继续做同 recipe 复核，或挂起等待别的路线对照。
...
出现 `strong_signal` 时，更合理的动作是优先复核同 recipe，而不是因为刚出现强信号就机械终止所有搜索。
...
一旦出现 `freeze_candidate`，就不应该继续无上限扩 recipe，而应先冻结它，转入人类决策：

- 继续追单折
- 还是转向 `fold1`
- 还是做多折 / rerank / ensemble

这里特意不把 `geom >= 20.0` 直接命名为 `aggressive_winner`。`aggressive_winner` 只保留给更后段、进入高阶 CV / ensemble 决策层时使用。
...
建议把交叉验证升级规则写成 promote 之后的业务分档决策，而不是核心阶段 gate：
...
- `Tier1`：`25 <= geom < 30`
  ...
  - 对应 `business_status = cv_queue_tier1`。
- `Tier2`：`30 <= geom < 35`
  - 仍以 `2-fold CV` 为主，并开始准备 OOF / ensemble 管线。
  - 若 `2-fold` 复现稳定，再决定是否继续扩折。
  - 对应 `business_status = cv_queue_tier2`。
- `Tier3`：`geom >= 35`
  - 直接进入 `5-fold CV`。
  - 这一档的目标不再是找方向，而是尽快榨取最终成绩。
  - 对应 `business_status = aggressive_winner`。

执行前提：

- 这里的 `geom` 指单折 full-val 主指标。
- 升档前至少完成一次同 recipe 复核，避免把偶然波动直接放大成多折成本。
- 这些 tier 只在 promote 已经 `stage_decision = accept` 之后才生效，不应用来替代第 7 节的阶段 gate。
...
需要把当前控制器正式扩成“双层输出”，而不是只返回一个简单结论。
...
- `business_status ∈ {observe, strong_signal, freeze_candidate, cv_queue_tier1, cv_queue_tier2, aggressive_winner}`
- `business_status` 只在 promote 已通过阶段 gate 后赋值，不应用来替代阶段决策
...
- `reserved`
- `nvidia-smi peak`
- `gpu_peak_utilization_pct`
- `gpu_utilization_p90_pct`（若暂不可得，允许先记 `null`，后续补齐）

并在 summary 中明确：

- 哪个数字用于自动 gate
- 哪个数字只作诊断参考
- 当前是否仍处于 `peak_fallback` 过渡口径

当前建议：

- 训练阶段自动 gate 以后续 `gpu_utilization_p90_pct + reserved` 为主
- decode 阶段优先看 `elapsed_seconds_per_sample + nvidia-smi peak + 输出健康指标`
- 在持续利用率统计尚未补齐前，允许暂时用 `gpu_peak_utilization_pct` 做过渡 gate，但必须明确落盘 `gpu_gate_metric = peak_fallback`

### 12.5 业务分层与后续排队规则代码化

应把下面这套规则直接写入控制器的 promote 后处理逻辑，而不是回头污染第 7 节的阶段 gate：

1. promote 阶段 gate 仍按第 7 节执行。

- `geom +0.30` 是 promote 通过线。
- `+0.30` 解决的是“值不值得留在主线挑战层”，不是“要不要立刻转多折”。

2. 当 promote 已经 `stage_decision = accept`，并至少完成一次同 recipe 复核后，再赋予业务状态：

- 默认 `business_status = observe`
- 单折相对 incumbent `geom +0.50` 以上，或绝对 `geom >= 14.5` 且 BLEU / chrF++ 同步向上：`business_status = strong_signal`
- 绝对 `geom >= 20.0` 且健康指标正常：`business_status = freeze_candidate`
- `25 <= geom < 30`：`business_status = cv_queue_tier1`
- `30 <= geom < 35`：`business_status = cv_queue_tier2`
- `geom >= 35`：`business_status = aggressive_winner`

3. 后续动作规则：

- `observe`：保留证据链，继续搜索或挂起对照
- `strong_signal`：优先做同 recipe 复核与补证，不自动停掉全局搜索
- `freeze_candidate`：默认暂停 recipe 扩张，等待人工确认是否转 `fold1 / CV / ensemble / rerank`
- `cv_queue_tier1`：进入 `2-fold CV` 排队
- `cv_queue_tier2`：继续 `2-fold CV`，并启动 OOF / ensemble 准备
- `aggressive_winner`：默认暂停 recipe 搜索，等待人工确认是否直接转 `5-fold CV / ensemble`
...
- 用更清晰的“阶段 gate / 业务分层”双层结构，保证激进但不乱烧
- 用分层目标把“冲 30”从口号拆成可执行里程碑
- 一旦出现 `freeze_candidate`、`cv_queue_tier1/2` 或 `aggressive_winner`，就先停下来，交给人类决定是否转折验证、多折、集成或 rerank

这套方案的核心不是“尽量少花钱”，而是“在高 credits 投入前提下，不浪费分毫算力，把每一小时都花在最可能涨分的地方”。

更准确地说，这是一套“受控自迭代”方案：

- 自动控制器负责高裁撤率筛选、阶段晋级与证据链记录
- promote 后的业务分层负责决定是否复核、冻结、转 CV 或转集成
- 人类只在真正高价值的节点接管，而不是在每一个小波动上被迫人工盯盘
